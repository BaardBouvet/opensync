...
| [engine/PLAN_ENGINE_API_ERGONOMICS.md](engine/PLAN_ENGINE_API_ERGONOMICS.md) | Ergonomics improvements for embedded engine use: `makeConnectorInstance()` factory, optional `conflict`/`readTimeoutMs`, per-channel conflict override, per-connector timeout, `bootChannel()`, `SyncEvent` export | draft | M |
...
| [engine/PLAN_RESOLUTION_STRATEGIES.md](engine/PLAN_RESOLUTION_STRATEGIES.md) | Resolve OSI-mapping 🔶-gaps for `collect` (tests + spec fix), `bool_or` (implement), and expression resolvers (incremental `resolve` function on `FieldMapping`) | backlog | S |
...
| [engine/PLAN_ARRAY_ORDERING.md](engine/PLAN_ARRAY_ORDERING.md) | Nested array ordering: `order_by` (custom field sort), `order: true` (CRDT ordinal via `_ordinal` injection), `order_linked_list: true` (linked-list reconstruction via `_prev`/`_next`); all applied at collapse time | backlog | M |
...
| [engine/PLAN_RECORD_FILTER.md](engine/PLAN_RECORD_FILTER.md) | Record-level `record_filter` / `record_reverse_filter` on mapping entries: exclude source records from resolution and canonical entities from reverse dispatch; enables discriminator routing and route-combined patterns | backlog | S |
...
| [engine/PLAN_FK_REFERENCE_TRANSLATION.md](engine/PLAN_FK_REFERENCE_TRANSLATION.md) | FK field translation pipeline: `references` config key, forward-pass local-ID → canonical-UUID (`dbGetCanonicalByExternalId`), reverse-pass canonical-UUID → local-ID (`dbGetExternalId`); merge-safe via runtime identity_map lookups | backlog | S |
| [engine/PLAN_SCALAR_ARRAYS.md](engine/PLAN_SCALAR_ARRAYS.md) | Scalar array expansion (`scalar: true`): expand bare-scalar JSON arrays into flat child records via `_value` field; forward pass only; reverse pass (collapse) deferred | backlog | S |
