...
| [engine/PLAN_SOURCE_PATH.md](engine/PLAN_SOURCE_PATH.md) | `source_path` dotted-path inline extraction: extract a nested JSON value by dotted path (e.g. `address.street`) or bracket index without a connector transform; reverse-path write-back merges sibling paths; array-index write-back disallowed | backlog | S |
