...
| [engine/PLAN_DELETE_PROPAGATION.md](engine/PLAN_DELETE_PROPAGATION.md) | Opt-in delete propagation: explicit signal (record.deleted = true) + mark-and-sweep; per-channel config, circuit breaker integration | complete | — |
...
| [engine/PLAN_PRIORITY_SEEDING.md](engine/PLAN_PRIORITY_SEEDING.md) | Priority-aware initial field seeding: `fieldMasters` hard-veto and `coalesce` priority ordering apply at `collectOnly` seeding time, not just on updates; prevents arrival-order non-determinism and master-owned fields being pre-seeded by non-master connectors | proposed | S |
...
| [engine/PLAN_SCALAR_ARRAY_COLLAPSE.md](engine/PLAN_SCALAR_ARRAY_COLLAPSE.md) | Scalar array collapse (reverse pass) + multi-level scalar chain support: set-rebuild strategy from canonical children, `_value` unwrap, `reverse_filter`, `_ordinal` sort; multi-level case navigates to intermediate parent element before rebuilding the leaf scalar array | complete | M |
| [engine/PLAN_INBOUND_OUTBOUND_RENAME.md](engine/PLAN_INBOUND_OUTBOUND_RENAME.md) | Rename all "forward/reverse" direction terms to "inbound/outbound": direction enum values, YAML keys (`reverse_expression` → `outbound_expression`, `reverse_filter` → `outbound_filter`, `reverseRequired` → `outbound_required`), TypeScript API, internal filter properties, spec narrative | proposed | M |
| [engine/PLAN_ROUTE_COMBINED.md](engine/PLAN_ROUTE_COMBINED.md) | Route combined validation: confirm that a filtered source and an unfiltered source merging into the same canonical entity via identity linking behaves correctly; test suite for shadow independence, ingest-order invariance, reverse-filter write-back suppression | complete | S |
| [engine/PLAN_ELEMENT_SET_RESOLUTION.md](engine/PLAN_ELEMENT_SET_RESOLUTION.md) | Element-set resolution: `elements_strategy: coalesce / last_modified` on array members; per-element conflict resolution across sources at collapse time; replaces implicit LWW by ingest order | complete | M |
| [engine/PLAN_ARRAY_ELEMENT_ASSOCIATIONS.md](engine/PLAN_ARRAY_ELEMENT_ASSOCIATIONS.md) | `ElementRecord` SDK type + `element()` factory: connector-supplied associations on array elements; Symbol-branded container lets engine extract per-element associations and optional element identity (`id`) without magic string keys | stale — needs rewrite | S |
...
| [engine/PLAN_HARD_DELETE.md](engine/PLAN_HARD_DELETE.md) | Hard delete detection: `full_snapshot: true` entity-absence detection (synthesize deleted records when IDs vanish from full-snapshot reads) + element-absence detection (clear stale child shadow rows after array expansion, force collapse write) | complete | M |
| [engine/PLAN_SOFT_DELETE_INSPECTION.md](engine/PLAN_SOFT_DELETE_INSPECTION.md) | Soft delete field inspection: `soft_delete:` mapping config with `deleted_flag`, `timestamp`, `active_flag`, and `expression` strategies; compiled at load time; sets `record.deleted = true` before the standard `_processRecords` path | complete | S |
...
| [engine/PLAN_ATOMIC_ARRAY.md](engine/PLAN_ATOMIC_ARRAY.md) | Atomic array pattern: document "map array as plain field, no array_path" as a first-class pattern; add `normalize: stable_sort` built-in for order-insensitive diff; add `element_fields` for per-element field mapping without expansion; spec §3.5 | complete | M |
| [engine/PLAN_SOURCE_PATH.md](engine/PLAN_SOURCE_PATH.md) | `source_path` dotted-path inline extraction: extract a nested JSON value by dotted path (e.g. `address.street`) or bracket index without a connector transform; reverse-path write-back merges sibling paths; array-index write-back disallowed | complete | S |
| [engine/PLAN_PASSTHROUGH_COLUMNS.md](engine/PLAN_PASSTHROUGH_COLUMNS.md) | Passthrough columns: `passthrough: [field, ...]` on a mapping entry forwards source fields to the canonical delta under namespaced `_passthrough.<connectorId>.<field>` keys; not dispatched to any target; participates in shadow diff for noop suppression | backlog | S |
| [engine/PLAN_EMBEDDED_OBJECTS.md](engine/PLAN_EMBEDDED_OBJECTS.md) | Embedded objects (flat parent mapping): `parent:` without `array_path` splits one source row into parent + child entity; deterministic child external ID; child fields merged into parent UpdateRecord on reverse; parent-deletion cascades to child tombstone | complete | M |
| [engine/PLAN_CONCURRENT_EDIT_DETECTION.md](engine/PLAN_CONCURRENT_EDIT_DETECTION.md) | Concurrent edit detection: `detect_concurrent_edits: true` on channel conflict config; emits `ConcurrentEditEvent` when ≥ 2 sources change the same field to different values in the same cycle; `onConcurrentEdit` subscriber; fieldMasters and group integration; advisory only — does not block resolution | backlog | M |
...
| [playground/PLAN_PLAYGROUND_MAPPING_PRIMITIVES.md](playground/PLAN_PLAYGROUND_MAPPING_PRIMITIVES.md) | Extend FIXED_SEED with fields needed to demonstrate all 16 uncovered implemented primitives; add mapping-showcase scenario exercising §1.2–§1.8, §2.1–§2.5, §3.5, §4.1, §5.1–§5.2, reverseRequired, soft_delete | backlog | M |
