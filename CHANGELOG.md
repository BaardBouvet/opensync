...
### Changed
- **Connector SDK — Write payloads carry plain strings** — FK reference fields in `InsertRecord.data` and `UpdateRecord.data` are now plain remapped ID strings (not `Ref` objects). `InsertRecord` and `UpdateRecord` gain an `associations?: Association[]` field populated by the engine, providing full FK metadata (predicate, targetEntity, targetId) for connectors that persist FK data in a separate format. `readRefs()` and `makeRefs()` are removed from the SDK — they are no longer needed. Specs updated: `specs/connector-sdk.md §Write Records`, `specs/associations.md §7.2–7.3`. HubSpot, SPARQL, and jsonfiles connectors updated.
