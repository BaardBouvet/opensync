/**
 * OpenSync POC v8 — SQLite database layer
 *
 * Extends v7 with:
 *   dbGetCanonicalFields  — read merged field values for a canonical_id from shadow_state
 *   dbGetLinkedConnectors — list connector_ids linked to a canonical_id
 *   dbGetAllCanonicals    — enumerate all canonical_ids linked to connectors in a channel
 */
...
function bootstrap(db: Db): void {
  ...
  // ── v7: onboarding log (append-only diagnostics) ────────────────────────────
...
// ─── Channel onboarding status (v7) ──────────────────────────────────────────

export type ChannelStatus = "uninitialized" | "ready";

export function dbGetChannelStatus(
  db: Db,
  channelId: string,
): ChannelStatus {
  const row = db
...
export function dbSetChannelReady(
  db: Db,
  channelId: string,
  entity: string,
): void {
...
export function dbLogOnboarding(
  db: Db,
  entry: {
    channelId: string;
    entity: string;
    action: "discover" | "onboard";
    matched?: number;
    uniqueCount?: number;
    linked?: number;
  ...
  db.run(
    `INSERT INTO onboarding_log
       (id, channel_id, entity, action, matched, unique_count, linked, shadows_seeded,
    ...
    [
      ...
      entry.action,
      entry.matched ?? null,
      entry.uniqueCount ?? null,
      entry.linked ?? null,
      entry.shadowsSeeded ?? null,
      entry.startedAt,
      entry.finishedAt,
    ],
  );
...
// ─── Canonical helpers (v8) ───────────────────────────────────────────────────

/**
 * Load all canonical_ids linked to any of the given connector_ids.
 * Returns them deduplicated (each canonical appears once).
 */
export function dbGetAllCanonicals(
  db: Db,
  connectorIds: string[],
): string[] {
  if (connectorIds.length === 0) return [];
  const placeholders = connectorIds.map(() => "?").join(", ");
  return db
    .query<{ canonical_id: string }, string[]>(
...
/**
 * List all connector_ids that have a row in identity_map for the given canonical_id.
 */
export function dbGetLinkedConnectors(
  db: Db,
  canonicalId: string,
): string[] {
  return db
...
/**
 * Return the merged canonical field values for a given canonical_id.
 * Reads shadow_state rows for all linked connectors and merges them:
 * first non-null value per field wins (priority by connector registration order,
 * which maps to the order rows appear in identity_map).
 */
export function dbGetCanonicalFields(
  db: Db,
  canonicalId: string,
  ...
  for (const row of rows) {
    const fd = JSON.parse(row.canonical_data) as import("./db.js").FieldData;
    for (const [k, entry] of Object.entries(fd)) {
...
/**
 * Return all non-deleted shadow_state rows for a given connector + entity.
 * Used by v9's DB-backed discover() — reads the last-ingested snapshot without
 * making any live calls to the source connector.
 */
export function dbGetAllShadowForEntity(
  ...
  return db
    .query<{ external_id: string; canonical_data: string }, [string, string]>(
      `SELECT external_id, canonical_data
       FROM shadow_state
